...
```bash
# 1) (Re)create the canonical notebook — payload is [title]; the CLI extracts the real UUID (response index [2]).
npx -y bun scripts/main.ts notebooks create --name GARY --json      # → { "id": "<uuid>", "url": "…/notebook/<uuid>" }

# 2) Re-ingest the base context that lives on disk (candidate-agnostic; regenerated by onboarding):
...
Notes:
- **Auth reuses the ONE automation browser `:9333`** (the same one all job-board connections use — it already
...
- The RPC auth works because `rpc-client.ts` `init()` follows redirects (refreshing the rotating
  `__Secure-1PSIDTS`); only the long-lived `__Secure-1PSID` must stay valid.
- **Existence check** (to decide reuse vs recreate): `notebooks exists <uuid> --json` → `{ "exists": bool }`.
  It uses LIST_NOTEBOOKS membership by UUID — do NOT use `sources list`, which returns `[]` for a DELETED
  notebook (can't tell "empty" from "gone").
- **Delete a notebook** (reverse-engineered): `DELETE_NOTEBOOK` id `WWINqb`, payload `[[<uuid>]]`, returns `[]`.
- **Delete a source** (reverse-engineered, VERIFIED): `DELETE_SOURCE` id `tGMBJ`, payload **`[[[<sourceId>]]]`**
  (triple-nested), returns `[]`. The old `[notebookId,[sourceId],[2]]` returned `[3]` and silently no-op'd.
