...
export class RPCClient {
  ...
  async init(): Promise<void> {
    // FOLLOW redirects with a cookie jar (re-applying Set-Cookie each hop). A freshly-captured Google
    // session (e.g. bridged from a logged-in browser) authenticates via ONE redirect hop where Google
    // refreshes __Secure-1PSIDTS before landing on the app — a single `redirect: 'manual'` fetch would
    // die on that first 302 even though the session is valid. Following the chain also updates
    // this.cookieMap with the rotated cookies, which subsequent execute() calls need.
    const res = await fetchWithCookieJar(NOTEBOOKLM_URL, { method: 'GET' }, this.cookieMap);

    // Still redirecting after the jar followed the whole chain → genuinely not authenticated.
    if (res.status >= 300 && res.status < 400) {
      const location = res.headers.get('location') ?? '';
      throw new Error(
