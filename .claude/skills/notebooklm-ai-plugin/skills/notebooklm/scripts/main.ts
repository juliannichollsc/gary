...
async function handleNotebooks(parsed: ParsedArgs): Promise<void> {
  ...
  if (sub === 'create') {
    const name = (parsed.flags.name as string) ?? (parsed.positional[0] as string) ?? 'GARY context';
    const cookieMap = await readCookieMapFromDisk({ log: (m) => { if (!isJson) textOutput(m); } });
    if (!hasRequiredCookies(cookieMap)) {
    ...
    await rpc.init();
    try {
      const nb = await createNotebook(rpc, name);
      await addNotebook({ id: nb.id, name, url: nb.url });
      await setActiveNotebook(nb.id);
      if (isJson) {
        jsonOutput(nb);
      } else {
        textOutput(`Created notebook "${name}"`);
        textOutput(`  ID:  ${nb.id}`);
        textOutput(`  URL: ${nb.url}`);
      }
    } catch (err) {
      errorOutput(`Failed to create notebook: ${err instanceof Error ? err.message : String(err)}`);
      process.exit(1);
    }
    return;
  }

  // Authoritative SERVER-side existence check by UUID (LIST_NOTEBOOKS membership). Used by GARY to decide
  // reuse-vs-recreate: `sources list` returns [] for a deleted notebook, so it can't tell "empty" from "gone".
  if (sub === 'exists') {
    const id = extractNotebookId((parsed.flags.notebook as string) ?? (parsed.positional[0] as string) ?? '');
    if (!id) { errorOutput('Usage: notebooks exists <id> [--json]'); process.exit(1); }
    const cookieMap = await readCookieMapFromDisk({ log: (m) => { if (!isJson) textOutput(m); } });
    if (!hasRequiredCookies(cookieMap)) {
      errorOutput('No valid cookies found. Run "login" first to authenticate with Google.');
      process.exit(1);
    ...
    const rpc = new RPCClient(cookieMap);
    await rpc.init();
    const exists = (await listRemoteNotebookIds(rpc)).includes(id);
    if (isJson) jsonOutput({ id, exists }); else textOutput(exists ? 'exists' : 'missing');
    return;
  }
