...
/**
 * Creates a NEW notebook on NotebookLM (server-side) and returns its id + url.
 * Uses the CREATE_NOTEBOOK RPC (CCqFvf). NOTE: the payload here is a best-effort `[title]`; if the API
 * rejects it, adjust the params after a live authenticated run (see the response shape via --json).
 */
export async function createNotebook(rpc: RPCClient, title: string): Promise<{ id: string; url: string }> {
  const response = await rpc.execute(RPC.CREATE_NOTEBOOK, [title], '/');
  // Verified response shape: [title, null, "<notebook-uuid>", …]. The real id is the UUID at index [2] —
  // NOT the first string (that's the echoed title). We validate a UUID and deep-scan as a fallback.
  const isUuid = (s: unknown): s is string =>
    typeof s === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(s);
  const scan = (data: unknown, depth = 0): string | null => {
    if (depth > 6) return null;
    if (isUuid(data)) return data;
    if (Array.isArray(data)) {
      for (const item of data) { const found = scan(item, depth + 1); if (found) return found; }
    }
    return null;
  };
  const id = (Array.isArray(response) && isUuid(response[2]) ? response[2] : null) ?? scan(response);
  if (!id) throw new Error('Could not extract the new notebook id from the CREATE_NOTEBOOK response.');
  return { id, url: `https://notebooklm.google.com/notebook/${id}` };
}

/**
 * Lists the SERVER-side notebook ids (LIST_NOTEBOOKS wXbhsf). This is the authoritative existence check:
 * `sources list` returns [] for a deleted notebook (looks empty, not missing), so to know whether a saved
 * notebook still exists you must check membership in this list by UUID. Response shape:
 *   [ [ ["", null, "<uuid>", "<title?>", …], … ] ]  → uuid is the first UUID-shaped string in each entry.
 */
export async function listRemoteNotebookIds(rpc: RPCClient): Promise<string[]> {
  const res = (await rpc.execute(RPC.LIST_NOTEBOOKS, [null, 1], '/')) as unknown;
  const entries = (Array.isArray(res) && Array.isArray(res[0]) ? (res[0] as unknown[]) : []);
  const isUuid = (s: unknown): s is string =>
    typeof s === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(s);
  const ids: string[] = [];
  for (const e of entries) {
    if (!Array.isArray(e)) continue;
    const id = (e as unknown[]).find(isUuid);
